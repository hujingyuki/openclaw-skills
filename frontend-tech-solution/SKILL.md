...
5. **重要**：文档创建成功后，**必须**调用 `feishu_perm action=add` 添加当前用户为协作者（权限：`full_access`）
6. **如果飞书 API 失败**，使用 `write` 保存到本地 `/当前工作区/output/` 目录
7. 返回文档链接（飞书或本地路径）
...
- `feishu_doc` — 创建飞书文档
- `feishu_perm` — 设置文档权限（**重要**：创建后必须添加当前用户为协作者）
- `write` — 生成临时文件
- `message` — 获取当前用户信息（用于权限设置）
...
- **权限设置（重要）**：
  - 文档创建后**必须**调用 `feishu_perm action=add` 添加当前用户为协作者
  - 权限级别使用 `full_access`（完全访问）
  - 如果当前用户是 Feishu Bot，使用 `member_type: "openid"` 和用户的 `open_id`
  - 如果无法获取用户 ID，使用 `member_type: "email"` 和用户的邮箱
